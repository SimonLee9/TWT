...
InitEQep(void)
{
    ...
    EQep1Regs.QEPCTL.bit.FREE_SOFT = 0x10;
    EQep1Regs.QEPCTL.bit.PCRM =0;               // QPOSCNT reset on index event
    EQep1Regs.QPOSMAX= 0xffffffff;              // 최대 위치 포인터 값 표시
    EQep1Regs.QEPCTL.bit.QPEN =1;               // QEP 위치 카운터 허용

    // Initialize eQEP2
    EQep2Regs.QDECCTL.bit.QSRC = 0x00;          // Quadrature count mode
    EQep2Regs.QDECCTL.bit.SWAP = 0;
    EQep2Regs.QEPCTL.bit.FREE_SOFT = 0x10;
    EQep2Regs.QEPCTL.bit.PCRM = 0;              // QPOSCNT reset on index event
    EQep2Regs.QPOSMAX = 0xffffffff;             // 최대 위치 포인터 값 표시
    EQep2Regs.QEPCTL.bit.QPEN = 1;              // QEP 위치 카운터 허용
}
